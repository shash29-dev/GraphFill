...
def visualize_mask_and_images_batch(batch: Dict[str, torch.Tensor], keys: List[str], max_items=10,
                                    last_without_mask=False, mask_only_first=True,rescale_keys=None) -> np.ndarray:
    ...
    for i in range(items_to_vis):
        ...
        result.append(visualize_mask_and_images(cur_dct, keys, last_without_mask=last_without_mask,mask_only_first=mask_only_first,
